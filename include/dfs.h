...
int DFS (std::shared_ptr<Board> BoardState, unsigned short& realOpenListSize, unsigned short& realExploredSize, int& maxRecursionDepth,
        unsigned short& numbersOfSteps, std::string& path, std::vector<uint_fast16_t>& pattern, uint_fast16_t maxRecurionsNumber) {
    ...
    while(!open_list.empty()){
        ...
        ++realExploredSize;
        explored[Hash<int>()(curNode->getState())] = curNode->getCounter();
        open_list.pop();
        --realOpenListSize;
        curNode->Reverse();
        for (const auto &it:curNode->getPossibleMovesForNode()){
            ...
            if(Children->getPath().length() >= maxRecurionsNumber)
                ...
                if(explored.at( Hash<int>()( Children->getState() )) <= Children ->getCounter() ) {
                    ...
                    explored.at(Hash<int>()(Children->getState())) = Children->getCounter();
                }
            }
